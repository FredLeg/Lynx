...
print("cdServer >>>>>>>>>>>>>>>> serverroot >>>",serverroot)
print("cdServer >>>>>>>>>>>>>>>> bin >>>",bin)
...
try:
    ...
    print("cdServer >>>>>>>>>>>>>>>> optlist >>>",optlist)
    print("cdServer >>>>>>>>>>>>>>>> args >>>",args)
except:
    usage(sys.stderr)
...
if debug:
    import lynx
    request_handler = lynx.SimpleHTTPRequestHandler
    lynx.checkACL = checkACL
    lynx.debug = True
else:
    try:
        import lynx
        ...
        lynx.checkACL = checkACL
        lynx.debug = False
        print "\n*** %s successfully initialized." % lynx.version
    except ImportError:
